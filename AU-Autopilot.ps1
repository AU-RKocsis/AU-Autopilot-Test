<#
    ...
    Version:        2.0
    Author:         Robert Kocsis
    Creation Date:  06/19/2026
    Purpose/Change: Added transcript logging via Start-Transcript; log is written to C:\Windows\Temp\AutopilotOOBE_<timestamp>.log for troubleshooting failed enrollments
...
##############################################################################################################
try {
    $LogFile = "$env:SystemRoot\Temp\AutopilotOOBE_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
